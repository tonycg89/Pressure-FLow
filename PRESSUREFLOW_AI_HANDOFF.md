...
- 07D-1 blocker: deployed estimate send to the owner-approved test inbox returned 502 Bad Gateway before a public estimate URL was persisted. Post-failure `/health` still returned `{"ok":true,"service":"pressureflow"}`, and the failed job remained `Lead` with no `estimateSentAt` and no `estimateApprovalUrl`.
- Current recommendation: no-go for external beta until the deployed estimate-send 502 is diagnosed/fixed and the remaining deployed sandbox checks are completed.
- Still blocked/manual after redeploy: Render log review for the 502, restart/redeploy persistence proof, public estimate link generation, contract/invoice/proof public link testing, Stripe/Square sandbox webhooks, Google OAuth/Calendar, Mapbox deployed workflow, and full deployed end-to-end workflow.
